Blacklist_Check(playerid, const type[], target[])
{
    ...
    if(cache_num_rows()) 
    {
        ...
        cache_get_value(0, "Username", username);
        cache_get_value(0, "Characters", characters);

        if(GetPVarInt(playerid, "ACPBlacklist"))
        {
            Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, "Ban Notice - ACP Banned", ""WHITE"Your ACP is banned from this server.\n\nUsername: "RED"%s\n"WHITE"Date: "RED"%s\n"WHITE"Banned by admin: "RED"%s\n"WHITE"Reason: "RED"%s\n\n"WHITE"To request a ban appeal, please visit our website and submit a ban appeal.", "Close", "", AccountData[playerid][pUsername], ConvertTimestamp(Time:date), banby, reason);
            DeletePVar(playerid, "ACPBlacklist");
        }
        else if(GetPVarInt(playerid, "IPBlacklist"))
        {
            Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, "Ban Notice - IP Banned", ""WHITE"Your IP is banned from this server.\n\nIP: "RED"%s\n"WHITE"Date: "RED"%s\n"WHITE"Banned by admin: "RED"%s\n"WHITE"Reason: "RED"%s\n\n"WHITE"To request a ban appeal, please visit our website and submit a ban appeal.", "Close", "", ReturnIP(playerid), ConvertTimestamp(Time:date), banby, reason);
            DeletePVar(playerid, "IPBlacklist");
        }
        else
        {
            if(!time)
                Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, "Ban Notice - Character Banned", ""WHITE"You are banned from this server.\n\nCharacter: "RED"%s\n"WHITE"Date: "RED"%s\n"WHITE"Banned by admin: "RED"%s\n"WHITE"Reason: "RED"%s\n\n"WHITE"To request a ban appeal, please visit our website and submit a ban appeal.", "Close", "", ReturnName(playerid), ConvertTimestamp(Time:date), banby, reason);

            else if((time) && (gettime() < time))
                Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, "Temp Ban - Character Banned", ""WHITE"You are banned from this server.\n\nCharacter: "RED"%s\n"WHITE"Date: "RED"%s\n"WHITE"Banned by admin: "RED"%s\n"WHITE"Reason: "RED"%s\n"WHITE"Unban Date: "RED"%s\n\n"WHITE"To request a ban appeal, please visit our website and submit a ban appeal.", "Close", "", ReturnName(playerid), ConvertTimestamp(Time:date), banby, reason, ConvertTimestamp(Time:time));
...
Blacklist_ACPExists(const type[], const target[])
{
    ...
    execute = mysql_query(g_iHandle, sprintf("SELECT `%s` FROM `accounts` WHERE `%s` = '%s' LIMIT 1;", type, type, SQL_ReturnEscaped(target)));
...
Blacklist_Add(userid, const ip[], const character[], const username[], const banner[], const reason[], day = 0)
{
    new query[500];
    ...
    if (userid != 0) {
        format(query, sizeof(query), "INSERT INTO `wanrslog` (`ID`, `warnBy`, `warnReason`, `warnDate`, `warnType`) VALUES ('%d','%s','%s','%s', '4')", userid, SQL_ReturnEscaped(banner), SQL_ReturnEscaped(reason), SQL_ReturnEscaped(ReturnDate()));
        mysql_tquery(g_iHandle, query);
    }
    return 1;
}
