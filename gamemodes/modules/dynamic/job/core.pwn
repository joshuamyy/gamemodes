...
Job_Sync(job_id)
{
	if(Job_Exists(job_id)) {
		if(IsValidDynamicPickup(JobData[job_id][jobPickup]))
			Job_UpdatePosition(job_id, JOB_POS_PICKUP);
        else 
        	JobData[jobid][jobPickup] = CreateDynamicPickup(1239, 23, JobData[job_id][jobPosition][0], JobData[job_id][jobPosition][1], JobData[job_id][jobPosition][2], 0, 0);

		if(IsValidDynamic3DTextLabel(JobData[job_id][jobLabel]))
			Job_UpdatePosition(job_id, JOB_POS_LABEL);
		else
        	JobData[jobid][jobLabel] = CreateDynamic3DTextLabel(sprintf("[%s (%d)]\n"WHITE"Gunakan "YELLOW"/takejob "WHITE"untuk mengambil pekerjaan!", Job_GetName(JobData[job_id][jobType]), job_id), COLOR_CLIENT, JobData[jobid][jobPosition][0], JobData[jobid][jobPosition][1], JobData[jobid][jobPosition][2]+0.5, 7.5, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0);

		if(IsValidDynamicMapIcon(JobData[job_id][jobMapIcon]))
			Job_UpdatePosition(job_id, JOB_POS_ICON);
		else
            JobData[jobid][jobMapIcon] = CreateDynamicMapIcon(JobData[jobid][jobPosition][0], JobData[jobid][jobPosition][1], JobData[jobid][jobPosition][2], 56, -1, 0, 0);
        ...
        if(JobData[job_id][jobType] == JOB_MECHANIC && JobData[job_id][jobType] == JOB_LUMBERJACK)
			if(IsValidDynamicPickup(JobData[job_id][jobPickupPoint]))
				Job_UpdatePosition(job_id, JOB_POS_PICKUP_POINT);
			else 
	        	JobData[jobid][jobPickupPoint] = CreateDynamicPickup(1239, 23, JobData[job_id][jobPoint][0], JobData[job_id][jobPoint][1], JobData[job_id][jobPoint][2], 0, 0);
