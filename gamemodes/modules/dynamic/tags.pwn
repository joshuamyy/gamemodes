...
#define 	TAG_DEFAULT_SIZE		(50)
#define 	TAG_DEFAULT_MAX_SIZE	(255)
...
stock Tags_Refresh(id) {
	...
	TagData[id][tagObject] = CreateDynamicObject(18659, TagData[id][tagPos][0], TagData[id][tagPos][1], TagData[id][tagPos][2], TagData[id][tagRot][0], TagData[id][tagRot][1], TagData[id][tagRot][2], _, _, _, 150, 150);
	SetDynamicObjectMaterialText(TagData[id][tagObject], 0, TagData[id][tagText], OBJECT_MATERIAL_SIZE_512x512, TagData[id][tagFont], TagData[id][tagFontsize], TagData[id][tagBold], RGBAToARGB(ColorList[TagData[id][tagColor]]), 0x00000000, TagData[id][tagAlignment]);
	return 1;
}
...
Function:Tags_Created(playerid, id) {
	...
	RefreshWeaponSlot(playerid, WEAPON_SPRAYCAN);
...
Function:Tags_Load() {
	for(new id; id != cache_num_rows(); id++) {
		cache_get_value_int(id, "tagId", TagData[id][tagId]);
		cache_get_value_int(id, "tagOwner", TagData[id][tagOwner]);
		cache_get_value_int(id, "tagBold", TagData[id][tagBold]);
		cache_get_value_int(id, "tagFontsize", TagData[id][tagFontsize]);
		cache_get_value_int(id, "tagColor", TagData[id][tagColor]);
		cache_get_value_int(id, "tagAlignment", TagData[id][tagAlignment]);

		cache_get_value(id, "tagText", TagData[id][tagText]);
		cache_get_value(id, "tagFont", TagData[id][tagFont]);
		cache_get_value(id, "tagCreated", TagData[id][tagCreated]);

		cache_get_value_float(id, "tagPosx", TagData[id][tagPos][0]);
		cache_get_value_float(id, "tagPosy", TagData[id][tagPos][1]);
		cache_get_value_float(id, "tagPosz", TagData[id][tagPos][2]);

		cache_get_value_float(id, "tagRotx", TagData[id][tagRot][0]);
		cache_get_value_float(id, "tagRoty", TagData[id][tagRot][1]);
		cache_get_value_float(id, "tagRotz", TagData[id][tagRot][2]);

		Iter_Add(Tags, id);
		Tags_Refresh(id);
	}
	printf("*** [R:RP Database: Loaded] tags data (%d count)", cache_num_rows());
	return 1;	
}
...
{
	if (EditingTag[playerid] != -1) {
		new id = EditingTag[playerid],Float:pos[3],Float:rot[3];
		...
		switch (response) {
			case EDIT_RESPONSE_CANCEL: {
				if (Iter_Contains(Tags, id))
				{
					...
					{
						Streamer_GetFloatData(STREAMER_TYPE_OBJECT,TagData[id][tagObject],E_STREAMER_X,pos[0]);
						Streamer_GetFloatData(STREAMER_TYPE_OBJECT,TagData[id][tagObject],E_STREAMER_Y,pos[1]);
						Streamer_GetFloatData(STREAMER_TYPE_OBJECT,TagData[id][tagObject],E_STREAMER_Z,pos[2]);
						Streamer_GetFloatData(STREAMER_TYPE_OBJECT,TagData[id][tagObject],E_STREAMER_R_X,rot[0]);
						Streamer_GetFloatData(STREAMER_TYPE_OBJECT,TagData[id][tagObject],E_STREAMER_R_Y,rot[1]);
						Streamer_GetFloatData(STREAMER_TYPE_OBJECT,TagData[id][tagObject],E_STREAMER_R_Z,rot[2]);
						SetDynamicObjectPos(objectid,pos[0],pos[1],pos[2]);
						SetDynamicObjectRot(objectid,rot[0],rot[1],rot[2]);

						SendCustomMessage(playerid, "TAGS", "You've canceled editing for this tag.");
...
CMD:tag(playerid, params[])
{
	...
	if(!strcmp(options, "create", true))
	{
		if (PlayerData[playerid][pScore] < 5) 
			return ShowPlayerFooter(playerid, "~r~ERROR: ~w~Level kamu tidak cukup untuk membuat tag, minimal level 5.");
		...
		Dialog_Show(playerid, TagName, DIALOG_STYLE_INPUT, "Masukkan teks ..", ""COL_LIGHTBLUE"HINT:\n"WHITE"- Teks mendukung bbcode, gunakan (e):untuk membuat baris baru.\n\
			...
			Maksimal teks tidak lebih dari %d huruf:", "Lanjut", "Keluar"
		, TAG_DEFAULT_MAX_SIZE);
	}
		...
		switch(listitem)
		{
			...
			case 1: {
				Dialog_Show(playerid, TagsText, DIALOG_STYLE_INPUT, "Masukkan teks ..", ""COL_LIGHTBLUE"HINT:\n"WHITE"- Teks mendukung bbcode, gunakan (n):untuk membuat baris baru, \
					...
					\tContoh: Ganti (r)saya (w)menjadi merah dan seterusnya putih\n\
					- Tidak memasukkan teks kepanjangan\n\n\
					Maksimal teks tidak lebih dari %d huruf:", "Ubah", "Keluar"
				, TAG_DEFAULT_MAX_SIZE);
			}
			case 2: 
			{
