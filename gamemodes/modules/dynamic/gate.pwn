...
Function:Gate_Load()
{
    ...
    for (new i = 0; i < rows; i ++) if(i < MAX_GATES)
    {
        ...
        cache_get_field_content(i, "gateMakeBy", GateData[i][gateMakeBy], g_iHandle, 32);
        cache_get_field_content(i, "gateLastEdit", GateData[i][gateLastEdit], g_iHandle, 32);

        Gate_Refresh(i);
    }
    printf("*** [R:RP Database: Loaded] gate data (%d count).", rows);
    return 1;
}
...
Gate_Save(gateid)
{
    ...
    format(query, sizeof(query), "UPDATE `gates` SET `gateModel` = '%d', `gateSpeed` = '%.4f', `gateRadius` = '%.4f', `gateTime` = '%d', `gateX` = '%.4f', `gateY` = '%.4f', `gateZ` = '%.4f', `gateRX` = '%.4f', `gateRY` = '%.4f', `gateRZ` = '%.4f', `gateInterior` = '%d', `gateWorld` = '%d', `gateMoveX` = '%.4f', `gateMoveY` = '%.4f', `gateMoveZ` = '%.4f', `gateMoveRX` = '%.4f', `gateMoveRY` = '%.4f', `gateMoveRZ` = '%.4f', `gateLinkID` = '%d', `gateFaction` = '%d', `gatePass` = '%s', `gateLastEdit`='%s', `gateMakeBy`='%s', `gateWorkshop`='%d' WHERE `gateID` = '%d'",
        GateData[gateid][gateModel], GateData[gateid][gateSpeed], GateData[gateid][gateRadius], GateData[gateid][gateTime],
