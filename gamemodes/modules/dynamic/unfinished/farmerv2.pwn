...
hook OnGameModeInitEx() {
	mysql_pquery(g_iHandle, "SELECT * FROM `plantfarm`", "Plant_Load", "");
	return 1;
}


hook OnGameModeExit() {
	foreach (new i : Plant) if (Iter_Contains(Plant, i)) {
		Plant_Save(i);
	}
	return 1;
}
...
Function:Plant_Load() {
	...
	for (new i = 0; i < rows; i ++) {
		Iter_Add(Plant, i);
		plantVariable[i][plantID] = cache_get_field_int(i, "ID");
		plantVariable[i][plantType] = cache_get_field_int(i, "Type");
		plantVariable[i][plantTime] = cache_get_field_int(i, "Time");

		for (new j = 0; j < 3; j ++) {
			plantVariable[i][plantPos][j] = cache_get_field_float(i, sprintf("Pos%d", j++));
		}

		RefreshPlant(i);
	}
	printf("*** [R:RP Database: Loaded] Plant data loaded (%d counts)", rows);
	return 1;
}

Function:OnPlantCreated(id) {
	if (!Iter_Contains(Plant, id))
		return false;

	plantVariable[id][plantID] = cache_insert_id(g_iHandle);
	Plant_Save(id);
	return true;
}
...
stock AddNewPlant(playerid, type) 
{
	...
	if ((plantid = Iter_Free(Plant)) != cellmin) {
		...
		mysql_tquery(g_iHandle, sprintf("INSERT INTO `plantfarm` (`Type`) VALUES ('%d')", type), "OnPlantCreated", "d", plantid);
...
stock Plant_Save(id) {
	...
	new query[512];
	format(query, sizeof(query), "UPDATE `plantfarm` SET `Type`='%d', `Time`='%d', `Pos1`='%f', `Pos2`='%f', `Pos3`='%f' WHERE `ID`='%d'",
	plantVariable[id][plantType],
	plantVariable[id][plantTime],
	plantVariable[id][plantPos][0],
	plantVariable[id][plantPos][1],
	plantVariable[id][plantPos][2],
	plantVariable[id][plantID]
	);
