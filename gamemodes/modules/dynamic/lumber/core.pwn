...
// Variable List
new LumberData[MAX_DYNAMIC_LUMBER][lumberData];
new Iterator:Lumbers<MAX_DYNAMIC_LUMBER>;

// Event List
hook OnGameModeInitEx()
{
    mysql_tquery(g_iHandle, sprintf("SELECT * FROM `lumber` ORDER BY `ID` ASC LIMIT %d;", MAX_DYNAMIC_LUMBER), "Lumber_Load", "");
    return 1;
}
...
public Lumber_Load()
{
    for (new id = 0; id < cache_num_rows(); id ++)
    {
        ...
        cache_get_value_int(id,"ID", LumberData[id][lumberID]);
        cache_get_value_int(id,"Tebang", LumberData[id][lumberCut]);
        cache_get_value_int(id,"Time", LumberData[id][lumberTime]);

        cache_get_value(id, "Pos", destinasi, sizeof(destinasi));
        sscanf(destinasi, "p<|>fff", LumberData[id][lumberPos][0], LumberData[id][lumberPos][1], LumberData[id][lumberPos][2]);

        cache_get_value(id, "Rot", destinasi, sizeof(destinasi));
        sscanf(destinasi, "p<|>fff", LumberData[id][lumberRot][0], LumberData[id][lumberRot][1], LumberData[id][lumberRot][2]);

        Lumber_Sync(id);
    }
    printf("*** [R:RP Database: Loaded] lumber data (%d count).", cache_num_rows());
    return 1;
}
...
Lumber_Delete(lumber_id)
{
    if(Lumber_Exists(lumber_id)) {
        ...
        LumberData[lumber_id][lumberID] = 0;
...
Lumber_UpdatePos(lumber_id, type)
{
    if(Lumber_Exists(lumber_id)) {
        if(type == 1) {
            ...
            new objectid = LumberData[lumber_id][lumberObject],
                cutting = LumberData[lumber_id][lumberCut];

            Streamer_SetFloatData(STREAMER_TYPE_OBJECT, objectid, E_STREAMER_X, LumberData[lumber_id][lumberPos][0]);
            Streamer_SetFloatData(STREAMER_TYPE_OBJECT, objectid, E_STREAMER_Y, LumberData[lumber_id][lumberPos][1]);
            Streamer_SetFloatData(STREAMER_TYPE_OBJECT, objectid, E_STREAMER_Z, (LumberData[lumber_id][lumberPos][2] - 1.0));

            Streamer_SetFloatData(STREAMER_TYPE_OBJECT, objectid, E_STREAMER_R_X, LumberData[lumber_id][lumberRot][0]);
            Streamer_SetFloatData(STREAMER_TYPE_OBJECT, objectid, E_STREAMER_R_Y, (cutting) ? (LumberData[lumber_id][lumberRot][1] - 80.0) : LumberData[lumber_id][lumberRot][1]);
            Streamer_SetFloatData(STREAMER_TYPE_OBJECT, objectid, E_STREAMER_R_Z, (cutting) ? (LumberData[lumber_id][lumberRot][2] + RandomFloat(0.0,360.0)) : LumberData[lumber_id][lumberRot][2]);
        }
        return 1;
    }
    return 0;
...
/* Lumber_SetCutting(lumber_id, bool:cutting)
{
    if(Lumber_Exists(lumber_id)) {
...
Show_Lumber(playerid) 
{
    ...
    new tree_str[64 * MAX_DYNAMIC_LUMBER], hour, minute, second, count;
