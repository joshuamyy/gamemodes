...
hook OnGameModeInitEx() {
  mysql_tquery(g_iHandle, "SELECT * FROM `objects`", "Object_Load", "");
  return 1;
}
...
Function:Object_Load() {
  ...
  printf("*** [R:RP Database: Loaded] object data loaded (%d count)", rows);
  return 1;
}
...
CMD:editobject(playerid, params[]) {
  ...
  if (sscanf(params, "s[64]S()[128]", opt, value)) {
    ...
    SendClientMessage(playerid, X11_YELLOW_2, "[OPTIONS]: "WHITE"position, fixpos, model, copy, gethere, material, resetmaterial, textprop, resettextprop, vw, int, drawdistance, house");
    return 1;
  }
  ...
  if (!strcmp(opt, "position")) {
    ...
    PlayerData[playerid][pEditingMode] = OBJECT;
    ...
    if (sscanf(value, "dd", id, houseid))
      return SendSyntaxMessage(playerid, "/editobject house [object id] [house id]");
    ...
    ObjData[id][oHouse] = HouseData[houseid][houseID];
    Object_Save(id);
    SendCustomMessage(playerid, "OBJECT", "You've changed house of object id: "YELLOW"%d "WHITE"to "YELLOW"%d",id,houseid);
  } else if (!strcmp(opt, "biz")) {
    new id, biz;
    if (sscanf(value, "dd", id, biz))
      return SendSyntaxMessage(playerid, "/editobject biz [object id] [biz id]");

    if (!Iter_Contains(Obj, id))
      return SendErrorMessage(playerid, "Invalid object id!");

    if (!BusinessData[biz][bizExists])
      return SendErrorMessage(playerid, "Invalid biz id!");

    ObjData[id][oBiz] = BusinessData[biz][bizID];
    Object_Save(id);
    SendCustomMessage(playerid, "OBJECT", "You've changed biz of object id: "YELLOW"%d "WHITE"to "YELLOW"%d",id,biz);
  }
  return 1;
}
