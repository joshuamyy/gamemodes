...
CMD:drop(playerid, params[])
{
    ...
    Log_Write("logs/dropweapon.txt", "[%s] %s has dropped a %s.", ReturnDate(), ReturnName(playerid, 0), ReturnWeaponName(weaponData[playerid][GetPlayerWeaponIndex(playerid)][weaponModel]));
...
CMD:givegun(playerid, params[])
{
    ...
    Log_Write("logs/give_log.txt", "[%s] %s (%s) has given a %s to %s (%s).", ReturnDate(), ReturnName(playerid, 0), PlayerData[playerid][pIP], ReturnWeaponName(weaponData[playerid][GetPlayerWeaponIndex(playerid)][weaponModel]), ReturnName(userid, 0), PlayerData[userid][pIP]);
