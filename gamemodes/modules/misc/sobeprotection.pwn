...
IRPC:VehicleDestroyed(playerid, BitStream:bs)
{
    ...
    if (GetVehicleModel(vehicleid) < 400)
    {
        ...
        Log_Write("logs/cheat_log.txt", "[%s] %s has try to destroy the vehicle (id: %d) with some hacks.", ReturnDate(), ReturnName(playerid, 0), vehicleid);
        return 0;
    }
...
public OnVehicleRequestDeath(vehicleid, killerid)
{
    ...
    Log_Write("logs/cheat_log.txt", "[%s] %s has try to destroy the vehicle (id: %d) with some hacks.", ReturnDate(), ReturnName(killerid, 0), vehicleid);
